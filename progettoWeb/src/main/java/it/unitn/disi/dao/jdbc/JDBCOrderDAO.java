...
public class JDBCOrderDAO extends JDBCDAO<Order, Integer> implements OrderDAO {
    ...
    public Order getOrderSeller(int id, int idSeller) throws DAOException {
        String query = "SELECT * FROM orders o JOIN shops s on o.id_shop = s.id WHERE o.id=? AND s.id_owner=? ORDER BY o.id DESC";
        Object[] parametriQuery = new Object[]{id, idSeller};
        Order o = DAOFunctions.getOne(query, parametriQuery, classe, nomiColonne, constructorParameterTypes, CON);
        setPointers(o, true, false, true);
        return o;
    }
    ...
    public Order[] getOrdersSeller(int idSeller) throws DAOException {
        String query = "SELECT * FROM orders o JOIN shops s on o.id_shop = s.id WHERE s.id_owner=? ORDER BY o.id DESC";
    ...
    public Order[] getOrdersShop(int idShop, int idSeller) throws DAOException {
        String query = "SELECT * FROM orders o JOIN shops s on o.id_shop = s.id WHERE o.id_shop=? AND s.id_owner=? ORDER BY o.id DESC";
        Object[] parametriQuery = new Object[]{idShop, idSeller};
        Order[] orders = DAOFunctions.getMany(query, parametriQuery, classe, nomiColonne, constructorParameterTypes, CON);
        for (Order o : orders) {
            setPointers(o, true, false, true);
        }
    ...
    //spedisce il prodotto o segna l'avvenuto ritiro in negozio
    @Override
    public boolean concludiOrder(int idOrder, int idShop) throws DAOException {
        String query = "UPDATE orders SET concluso = ? , datetime_concluso = ? WHERE id = ? AND id_shop = ?;";
        try (PreparedStatement ps = CON.prepareStatement(query)) {
            ...
            try {
                ...
                //System.err.println("Impossibile eseguire query: " + ex.getMessage());
                throw new DAOException("Errore SQLException esecuzione query concludiOrder: " + ex.getMessage());
            }
            boolean b = (result > 0); //se ha modificato almeno 1 riga, restituisce true
            return b;
        } catch (SQLException ex) {
            throw new DAOException("Errore SQLExcpetion in concludiOrder: " + ex.getMessage());
        }
    }
